  ...
  return new Promise(resolve => {
    ...
    $.get({ url: 'https://api.ninesix.cc/api/jx-cfd' }, async (err, resp, _data) => {
      try {
        ...
        $.log(`\n【👫🏻超级助力】超级助力码：${'Jxcfd_GroupId_138_20887290'}\n${$.showLog ? _data : ''}`);
        $.get(taskUrl('user/JoinScene', `strPgtimestamp=${$.currentToken['timestamp']}&strPhoneID=${$.currentToken['phoneid']}&strPgUUNum=${$.currentToken['farm_jstoken']}&strShareId=${escape('72C590FC794C19D188CC47A79D1A004EB44E0E7795A3ABA06E72E7F5A4AE1837')}&dwSceneId=${sceneId}&dwType=2`), async (err, resp, data) => {
          try {
  ...
  return new Promise(resolve => {
    ...
    $.get({ url: 'https://api.ninesix.cc/api/jx-cfd' }, async (err, resp, _data) => {
      try {
        ...
        $.get(taskUrl('user/JoinScene', `strShareId=${escape('72C590FC794C19D188CC47A79D1A004EB44E0E7795A3ABA06E72E7F5A4AE1837')}&dwSceneId=${sceneId}`), async (err, resp, data) => {
          try {
            const { sErrMsg, data: { rewardMoney = 0 } = {} } = JSON.parse(data);
            $.log(`\n【👬普通助力】助力：${sErrMsg}\n${$.showLog ? data : ''}`);
          } catch (e) {
  ...
  return new Promise( async (resolve) => {
    $.get({ url: 'https://api.ninesix.cc/api/jx-cfd-group' }, (err, resp, _data) => {
      try {
        ...
        $.get(taskUrl(`user/JoinGroup`, `strGroupId=${'Jxcfd_GroupId_138_20887290'}&dwIsNewUser=${$.info.dwIsNewUser}&pgtimestamp=${$.currentToken['timestamp']}&phoneID=${$.currentToken['phoneid']}&pgUUNum=${$.currentToken['farm_jstoken']}`), (err, resp, data) => {
          try {
            const { sErrMsg } = JSON.parse(data);
            $.log(`\n【🏝寻宝大作战】助力：${sErrMsg}\n${$.showLog ? data : ''}`);
          } catch (e) {
