...
//
async function isSuccessCheckin(){
 return new Promise((resolve) => {
   ...
   $.post(checkinInit_url,async(error, response, data) =>{
      try{
            const result = JSON.parse(data)
            if(logs)$.log(data)
            if(result.data.todayTask.type == 1){
               
              console.log(` 今日已打卡成功：获得${result.data.curMoney}元\n`)
